...
/**
 * React Bits MetallicPaint — liquid-metal shader over a black-fill shape image.
 * Ported as-is except the canvas is sized to its container (not a forced 1000px
 * square) so a wide wordmark fits, with `u_ratio` fed the real canvas aspect.
 */
...
export function MetallicPaint({
  ...
  // Size the canvas to its container (not a forced square) and feed the shader
  // the real canvas aspect via u_ratio, so a wide wordmark isn't letterboxed.
  useEffect(() => {
    if (!initGL()) return;
    const canvas = canvasRef.current;
    const gl = glRef.current;
